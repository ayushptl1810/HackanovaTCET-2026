...
Internals: slab-based scoring against the Redis scheme cache.
"""
...
def _load_schemes_from_redis() -> list:
    """Load schemes from Redis if available, else fall back to file."""
    try:
        import redis
    ...
    fallback_path = os.path.join(os.path.dirname(__file__), "..", "schemes_database.json")
...
def get_top_schemes(
    ...
    """
    schemes = _load_schemes_from_redis()
    if not schemes:
